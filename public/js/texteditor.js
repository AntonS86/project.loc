...
/***/ "./resources/js/Article/texteditor.js":
/*!********************************************!*\
  !*** ./resources/js/Article/texteditor.js ***!
  \********************************************/
/*! no exports provided */
...
/* harmony import */ var _custom_Notify__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../custom/Notify */ "./resources/js/custom/Notify.js");
...
var notify = new _custom_Notify__WEBPACK_IMPORTED_MODULE_1__["default"]();
...
(function () {
  var setting = {
    ...
    input: ['_method', 'title', 'desc', 'meta_desc', 'keywords', 'category_id'],
          ...
          try {
            for (var _iterator = files[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              ...
              if (setting.imageRegexp.test(file.type) && setting.imageNameRegexp.test(file.name)) {
                ...
                notify.alertMessage('Не верное имя или формат файла');
              }
            }
          } catch (err) {
  ...
  function insertToEditor(response) {
    if (response.image) {
      ...
      notify.alertMessage('Ошибка загрузки файла');
    }
  }
  /**
  ...
  function ajaxFileSave(file, inputName, setting, responseHandler) {
    ...
    ajax.setRequestHeader('X-CSRF-TOKEN', setting.csrf); //ajax.setRequestHeader('Content-type', 'application/json; charset=utf-8');
    ...
    ajax.onload = function () {
      if (ajax.status === 200) {
        ...
        var errors = ajax.response.errors;

        for (var key in errors) {
          errors[key].forEach(function (element) {
            notify.alertMessage(element);
          });
        }
      } else {
        notify.alertMessage('Ошибка сохранения файла на сервере, попробуйте авторизоваться');
      }
    };

    ajax.onerror = function () {
      notify.alertMessage('Ошибка сохранения файла на сервере');
    };
  ...
  function addTitleImage(sTitleImg, sPreviewImg) {
    if (selectorError(sTitleImg, sPreviewImg)) {
      var inputTitleImage = document.querySelector(sTitleImg);
      var titleImagePreview = document.querySelector(sPreviewImg);
      inputTitleImage.addEventListener('change', function (e) {
        ...
        if (file[0] && setting.imageRegexp.test(file[0].type) && setting.imageNameRegexp.test(file[0].name)) {
          ...
          notify.alertMessage('Поддерживаемый формат изображения jpeg jpg png gif');
  ...
  function loadTitleImage(_x, _x2) {
    return _loadTitleImage.apply(this, arguments);
  }

  function _loadTitleImage() {
    _loadTitleImage = _asyncToGenerator(
    /*#__PURE__*/
    _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee(file, collable) {
      var formData, response, result, key;
          ...
          switch (_context.prev = _context.next) {
            ...
            case 8:
              ...
              if (response.status == 200) {
                collable(result); //console.log(result);
                ...
                for (key in result.errors) {
                  result.errors[key].forEach(function (element) {
                    notify.alertMessage(element);
                  });
                }
              }
  ...
  function updateForm(e) {
    e.preventDefault();
    var data = dataPreparation(setting.form, setting.input);
    ...
    ajax.onload = function () {
      if (ajax.status === 200) {
        notify.infoMessage(ajax.response.status);
        ...
        notify.alertMessage('Ошибка сохранения данных на сервере, попробуйте авторизоваться');
      }
    };

    ajax.onerror = function () {
      notify.alertMessage('Ошибка сохранения данных на сервере');
    };
  ...
  function dataPreparation(form, input) {
    ...
    status();
    textReplaceClass(); //data.text = document.querySelector('.ql-editor').innerHTML;
  ...
  function textReplaceClass() {
    ...
    for (var needle in setting.classNameAdd) {
...
/***/ 4:
/*!**************************************************!*\
  !*** multi ./resources/js/Article/texteditor.js ***!
  \**************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! Z:\OSPanel\domains\proj.loc\resources\js\Article\texteditor.js */"./resources/js/Article/texteditor.js");
